...
@dag(
    dag_id='cricsheet_gcs_full_idempotency_check',
...
def cricsheet_full_idempotency_pipeline():
    ...
    for match_type, config in CRICSHEET_SOURCES.items():
        ...
        check_task = check_if_source_updated(
            url=config["url"],
            etag_variable_key=config["etag_variable"]
        )
        
        process_task = process_and_load_data(
            url=config["url"],
...
# Instantiate the DAG
cricsheet_full_idempotency_pipeline()